...
public:
    ...
    //
    UBlox   ();
    UBlox   (TWI& Wire,uint8_t address);
    ...
    void    GetPeriodic();
		uint32_t write(uint8_t _address, uint8_t payload);
		uint32_t read(uint8_t _address, uint8_t* buf, uint16_t bytes2Read);
    ...
    // function pointers
    void    (*funcNavPvt) (NavigationPositionVelocityTimeSolution*);
