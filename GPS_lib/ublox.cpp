...
UBlox::UBlox ():
    Wire_(Wire),
		(*funcNavPvt) (NavigationPositionVelocityTimeSolution*)(NULL){
    address_ = 0x42;
    //
...
UBlox::UBlox (TWI& aWire,uint8_t address):
    Wire_(aWire),
		(*funcNavPvt) (NavigationPositionVelocityTimeSolution*)(NULL){
    address_ = address;
    //
...
void UBlox::flush() {
    ...
    if (bytes) {
			  Wire.Read(address_, uint8_t * rx_buf, bytes);
...
uint16_t UBlox::available() {
    write(address_, (uint8_t)0xFD);
	  uint8_t rx_data[2];
		read(address_, rx_data, sizeof(rx_data));
		uint16_t totalBytes = ((uint16_t) rx_data[0] << 8) | rx_data[1];
    return totalBytes;
}

uint32_t UBlox::write(uint8_t _address, uint8_t payload){
	uint32_t err_code = nrf_drv_twi_tx(&Wire_, _address, &payload, sizeof(payload), true);
	nrf_delay_ms(100);
	return err_code;
}

uint32_t UBlox::read(uint8_t _address, uint8_t* buf, uint16_t bytes2Read){
	uint32_t err_code = nrf_drv_twi_rx(&Wire_, _address, (uint8_t*) buf, bytes2Read);
	nrf_delay_ms(100);
	return err_code;
}
